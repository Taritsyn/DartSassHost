...
var SassHelper = (function (sass, fileManager, currentOsPlatformName, undefined) {
	...
	modules['/buffer'] = function () {
		 ...
		 *
		 * Copyright (c) 2012-2020, Feross Aboukhadijeh <https://feross.org>
		 * Licensed under the MIT License.
		 *
		 */
	...
	//#region URL: sass-helper
	modules['/sass-helper'] = function () {
		var dshUtils,
			DshFileManagerProxy,
			DshCustomLogger,
			DshNullLogger
		...
		dshUtils = (function () {
			...
			function processBackSlashes(path) {
				if (!path) {
					return '';
				}
			...
			function processForwardSlashes(path) {
				if (!path) {
					return '';
				}
			...
			function getAbsolutePathFromUri(uri) {
				...
				path = uri.toString();
				processedPath = fixAbsolutePath(path);
			...
			function fixAbsolutePath(path) {
				...
				processedPath = fixPathWithDriveLetter(processedPath);
				processedPath = processForwardSlashes(processedPath);

				return processedPath;
			}

			function fixAbsoluteUrl(path) {
				var processedPath;
			...
			function fixPathWithDriveLetter(path) {
				var processedPath = path,
					hasDriveLetter,
					driveLetter,
					driveLetterInUpperCase,
					pathWithoutDriveLetter
					;

				hasDriveLetter = pathWithDriveLetterRegEx.test(processedPath);
				if (hasDriveLetter) {
			...
			exports.fixAbsoluteUrl = fixAbsoluteUrl;
		...
		SassHelper = (function () {
			...
			function SassHelper(options) {
				...
				this._compilationOptions = getCommonCompilationOptions(options);
				this._sourceMapOptions = getCommonSourceMapOptions(options);
			}
			...
			SassHelper.prototype.compile = function (content, indentedSyntax, inputFilePath, outputFilePath, sourceMapFilePath, options) {
				return this._innerCompile(content, indentedSyntax, inputFilePath, outputFilePath, sourceMapFilePath, options);
			};

			SassHelper.prototype.compileFile = function (inputFilePath, outputFilePath, sourceMapFilePath, options) {
				return this._innerCompile(undefined, false, inputFilePath, outputFilePath, sourceMapFilePath, options);
			};

			SassHelper.prototype._innerCompile = function (content, indentedSyntax, inputFilePath, outputFilePath, sourceMapFilePath, options) {
				var result,
					inputFileAbsolutePath,
					inputFileFullPath,
					outputFileAbsolutePath,
					sourceMapFileAbsolutePath,
					compilationOptions,
					sourceMapOptions,
					compilationResult,
					compiledContent = '',
				...
				inputFileAbsolutePath = fileManagerProxy.convertPathToAbsolute(inputFilePath);
				outputFileAbsolutePath = fileManagerProxy.convertPathToAbsolute(outputFilePath);
				sourceMapFileAbsolutePath = fileManagerProxy.convertPathToAbsolute(sourceMapFilePath);
				compilationOptions = getAdvancedCompilationOptions(content, indentedSyntax, inputFileAbsolutePath, options,
					this._compilationOptions);
				sourceMapOptions = getAdvancedSourceMapOptions(inputFileAbsolutePath, outputFileAbsolutePath,
					sourceMapFileAbsolutePath, options, this._sourceMapOptions);
				...
				{
					if (content) {
						compilationResult = sass.compileString(content, compilationOptions);
					}
					else {
						inputFileFullPath = getFullPath(inputFileAbsolutePath);
						compilationResult = sass.compile(inputFileFullPath, compilationOptions);
					}
					addAdditionalSourceMapFeaturesToCompilationResult(compilationResult, sourceMapOptions);
					compiledContent = compilationResult.css;
					sourceMap = compilationResult.sourceMap;
					includedFilePaths = cleanLoadedUrls(compilationResult.loadedUrls);
			...
			function getCommonCompilationOptions(options) {
				var compilationOptions = {
					'charset': options.charset,
					'fatalDeprecations': fixFatalDeprecations(options.fatalDeprecations),
			...
			function getAdvancedCompilationOptions(content, indentedSyntax, inputFilePath, options, defaultCompilationOptions) {
				...
				if (options) {
					compilationOptions = getCommonCompilationOptions(options);
				}

				if (content) {
					compilationOptions.syntax = indentedSyntax ? 'indented' : 'scss';
					if (inputFilePath) {
						inputFileFullPath = getFullPath(inputFilePath);
						inputFileFullUrl = convertPathToUrl(inputFileFullPath);
						compilationOptions.url = inputFileFullUrl;
					}
				}
			...
			function getCommonSourceMapOptions(options) {
			...
			function getAdvancedSourceMapOptions(inputFilePath, outputFilePath, sourceMapFilePath, options, defaultSourceMapOptions) {
				var sourceMapOptions = defaultSourceMapOptions,
					context
					;

				if (!outputFilePath) {
					context = sass.forDsh.getContext();
					outputFilePath = context.withoutExtension$1(outputFilePath) + '.css';
				}
				...
				if (options) {
					sourceMapOptions = getCommonSourceMapOptions(options);
				}
				sourceMapOptions.outFile = outputFilePath;
				sourceMapOptions.sourceMapFile = sourceMapFilePath;

				return sourceMapOptions;
			}

			function addAdditionalSourceMapFeaturesToCompilationResult(compilationResult, sourceMapOptions) {
				var compiledContent = compilationResult.css || '',
					sourceMap = compilationResult.sourceMap
					;
				...
				sourceMap.sourceRoot = sourceMapOptions.sourceMapRoot;
				addOutputFileUrlToSourceMap(sourceMap, sourceMapOptions);
				makeRelativeSourceFileUrls(sourceMap.sources, sourceMapOptions);

				var sourceMapText = JSON.stringify(sourceMap);
				compiledContent = getCompiledContentWithSourceMapUrl(compiledContent, sourceMapText, sourceMapOptions);

				compilationResult.css = compiledContent;
				compilationResult.sourceMap = sourceMapText;
			}

			function addOutputFileUrlToSourceMap(sourceMap, sourceMapOptions) {
				var outputFilePath,
					outputFileUrl,
					sourceMapFilePath,
					sourceMapFileUrl,
					sourceMapDirUrl,
					outputFileUrlRelativeToSourceMapFile,
					sourcesContent
					;

				outputFilePath = sourceMapOptions.outFile;
				outputFileUrl = convertPathToUrl(outputFilePath);
				sourceMapFilePath = sourceMapOptions.sourceMapFile;
				sourceMapFileUrl = convertPathToUrl(sourceMapFilePath);
				sourceMapDirUrl = getDirectoryUrl(sourceMapFileUrl);
				outputFileUrlRelativeToSourceMapFile = getRelativeUrl(sourceMapDirUrl, outputFileUrl);
				sourcesContent = sourceMap.sourcesContent;

				if (sourcesContent) {
					delete sourceMap.sourcesContent;
					sourceMap.file = outputFileUrlRelativeToSourceMapFile;
			...
			function makeRelativeSourceFileUrls(sources, sourceMapOptions) {
				var sourceMapFilePath,
					sourceMapFileUrl,
					sourceMapDirUrl,
					isSourceMapDirUrlRelative,
					currentDirPath,
					currentDirUrl,
					sourceIndex,
					sourceFileUrl,
				...
				if (sources) {
					...
					sourceMapFileUrl = convertPathToUrl(sourceMapFilePath);
					sourceMapDirUrl = getDirectoryUrl(sourceMapFileUrl);
					isSourceMapDirUrlRelative = isRelativeUrl(sourceMapDirUrl);

					if (isSourceMapDirUrlRelative) {
						currentDirPath = sass.dsh.fileManagerProxy.getCurrentDirectory();
						currentDirUrl = convertPathToUrl(currentDirPath);
					}

					for (sourceIndex = 0; sourceIndex < sources.length; sourceIndex++) {
						sourceFileUrl = sources[sourceIndex];

						if (sourceFileUrl === 'stdin' || isRelativeUrl(sourceFileUrl)) {
							continue;
						}

						sourceFileRelativeUrl = sourceFileUrl;
						if (isSourceMapDirUrlRelative) {
							sourceFileRelativeUrl = getRelativeUrl(currentDirUrl, sourceFileRelativeUrl);
						}
						sourceFileRelativeUrl = getRelativeUrl(sourceMapDirUrl, sourceFileRelativeUrl);

						sources[sourceIndex] = sourceFileRelativeUrl;
					}
				}
			}

			function getCompiledContentWithSourceMapUrl(compiledContent, sourceMapText, sourceMapOptions) {
				var compiledContentWithSourceMapUrl = compiledContent,
					Buffer,
					sourceMapUrl,
					outputFilePath,
					outputFileUrl,
					...
					sourceMapFileUrl,
					sourceMapBytes,
					sourceMapBase64String
					;

				if (!sourceMapOptions.omitSourceMapUrl) {
					if (sourceMapOptions.sourceMapEmbed) {
						Buffer = require('/buffer').Buffer; //DSH+
						sourceMapBytes = Buffer.from(sourceMapText);
						sourceMapBase64String = sourceMapBytes.toString('base64');
						sourceMapUrl = 'data:application/json;base64,' + sourceMapBase64String;
					}
					else {
						outputFilePath = sourceMapOptions.outFile;
						sourceMapFilePath = sourceMapOptions.sourceMapFile;
						...
						if (outputFilePath) {
							outputFileUrl = convertPathToUrl(outputFilePath);
							outputDirUrl = getDirectoryUrl(outputFileUrl);
							sourceMapUrl = getRelativeUrl(outputDirUrl, sourceMapFileUrl);
						}
						else {
							sourceMapUrl = sourceMapFileUrl;
						}
					}
					compiledContentWithSourceMapUrl += '\n\n/*# sourceMappingURL=' + sourceMapUrl.replace(/\*\//g, '%2A/') + ' */';
				}

				return compiledContentWithSourceMapUrl;
			}

			function getFullPath(pathOrUrl) {
				var fullPath,
					context
					;
			...
			function getDirectoryUrl(url) {
				...
				urlHelper = sass.forDsh.getUrl();
				dirUrl = urlHelper.dirname$1(url);

				return dirUrl;
			}

			function getRelativeUrl(relativeTo, url) {
				var relativeUrl,
					urlHelper
					;

				urlHelper = sass.forDsh.getUrl();
				relativeUrl = urlHelper.relative$2$from(url, relativeTo);
			...
			function isRelativeUrl(url) {
				var isRelative,
					...
					urlHelper = sass.forDsh.getUrl();
					urlStyleHelper = urlHelper.style;
					isRelative = urlStyleHelper.rootLength$1(url) <= 0
						|| urlStyleHelper.isRootRelative$1(url);
			...
			function convertPathToUrl(path) {
				...
				context = sass.forDsh.getContext();
				url = context.toUri$1(path).toString$0(0);

				return url;
			}

			function cleanLoadedUrls(urls) {
				var cleanedUrls,
					fileManagerProxy,
					currentDirPath,
					canonicalCurrentDirPath,
					url,
					urlIndex,
					canonicalPath,
					fixAbsoluteUrlFunc
				...
				cleanedUrls = [];
				fileManagerProxy = sass.dsh.fileManagerProxy;
				currentDirPath = fileManagerProxy.getCurrentDirectory();
				canonicalCurrentDirPath = dshUtils.getCanonicalFilePath(currentDirPath);
				fixAbsoluteUrlFunc = fileManagerProxy.supportsVirtualPaths ?
					dshUtils.fixAbsoluteUrl : dshUtils.fixAbsolutePath;

				for (urlIndex = 0; urlIndex < urls.length; urlIndex++) {
					url = fixAbsoluteUrlFunc(urls[urlIndex]);
					if (!url) {
						continue;
					}

					canonicalPath = dshUtils.getCanonicalFilePath(url);
					if (canonicalPath !== canonicalCurrentDirPath) {
			...
			function mapError(error) {
				...
				newError = {
					...
					'type': type || 'Error',
					'file': dshUtils.getAbsolutePathFromUri(uri),
					'lineNumber': line || 0,
				...
				if (stackFrames && stackFrames.length > 0) {
					newError.stackFrames = stackFrames;
				}
