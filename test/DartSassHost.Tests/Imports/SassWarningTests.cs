...
namespace DartSassHost.Tests.Imports
{
	...
	public sealed class SassWarningTests : WarningTestsBase
	{
		...
		public void UsageOfFatalDeprecationsPropertyDuringCompilation([Values] bool fromFile)
		{
			...
			{
				FatalDeprecations = new List<string> { "1.3.2", DeprecationId.ColorModuleCompat },
				IncludePaths = alternativePaths
			};
			...
			using (var sassCompiler = CreateSassCompiler())
			{
				...
				string description3 = WarningConstants.SassImportRulesDeprecated +
					"\n\n" +
					string.Format(WarningConstants.DeprecationWarningAsErrorExplanation, DeprecationId.Import)
					;
				Assert.AreEqual(
		...
		{
			// Arrange
			var options = new CompilationOptions { SilenceDeprecations = new List<string> { DeprecationId.Import } };
			...
			Assert.AreEqual(
				...
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:11:10) -> " +
				"  width: math.div(100% * $columns - $grid-gutter-width * ($grid-columns - $columns), \"#{$grid-column" + Environment.NewLine +
				"   at root stylesheet (Files/imports/warnings/division-with-non-numeric-args/sass/base.sass:8:5)",
				warnings[0].Message
			);
			...
			Assert.AreEqual(11, warnings[0].LineNumber);
			Assert.AreEqual(10, warnings[0].ColumnNumber);
			Assert.AreEqual(
				"Line 10:   float: left" + Environment.NewLine +
				"Line 11:   width: math.div(100% * $columns - $grid-gutter-width * ($grid-columns - $columns), \"#{$grid…" + Environment.NewLine +
				"------------------^" + Environment.NewLine +
				"Line 12:   margin-right: math.div(\"#{$grid-gutter-width}\", 2)",
				warnings[0].SourceFragment
			);
			Assert.AreEqual(
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:11:10)" + Environment.NewLine +
				"   at root stylesheet (Files/imports/warnings/division-with-non-numeric-args/sass/base.sass:8:5)",
			...
			Assert.AreEqual(
				"Warning: " + WarningConstants.MathDivOnlySupportNumberArguments + Environment.NewLine +
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:12:17) -> " +
			...
			Assert.AreEqual(12, warnings[1].LineNumber);
			Assert.AreEqual(17, warnings[1].ColumnNumber);
			Assert.AreEqual(
				"Line 11:   width: math.div(100% * $columns - $grid-gutter-width * ($grid-columns - $columns), \"#{$grid-c…" + Environment.NewLine +
				"Line 12:   margin-right: math.div(\"#{$grid-gutter-width}\", 2)" + Environment.NewLine +
				"-------------------------^" + Environment.NewLine +
				"Line 13:   margin-left: math.div(\"\" + $grid-gutter-width, 2)",
				warnings[1].SourceFragment
			);
			Assert.AreEqual(
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:12:17)" + Environment.NewLine +
				"   at root stylesheet (Files/imports/warnings/division-with-non-numeric-args/sass/base.sass:8:5)",
			...
			Assert.AreEqual(
				"Warning: " + WarningConstants.MathDivOnlySupportNumberArguments + Environment.NewLine +
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:13:16) -> " +
			...
			Assert.AreEqual(13, warnings[2].LineNumber);
			Assert.AreEqual(16, warnings[2].ColumnNumber);
			Assert.AreEqual(
				"Line 12:   margin-right: math.div(\"#{$grid-gutter-width}\", 2)" + Environment.NewLine +
				"Line 13:   margin-left: math.div(\"\" + $grid-gutter-width, 2)" + Environment.NewLine +
				"------------------------^",
				warnings[2].SourceFragment
			);
			Assert.AreEqual(
				"   at make-column (Files/imports/warnings/division-with-non-numeric-args/sass/mixins/_grid.sass:13:16)" + Environment.NewLine +
				"   at root stylesheet (Files/imports/warnings/division-with-non-numeric-args/sass/base.sass:8:5)",
		...
		public void MappingSassDeprecationWarningDuringCompilation([Values]bool fromFile)
		{
			...
			Assert.AreEqual(
				"Deprecation Warning [" + DeprecationId.Import + "]: " + WarningConstants.SassImportRulesDeprecated + Environment.NewLine +
			...
			Assert.AreEqual(DeprecationId.Import, warnings[0].DeprecationId);
			...
			Assert.AreEqual(
				"Deprecation Warning [" + DeprecationId.GlobalBuiltin + "]: " + description3 + Environment.NewLine +
			...
			Assert.AreEqual(DeprecationId.GlobalBuiltin, warnings[2].DeprecationId);
		...
		public void MappingSassCustomWarningDuringCompilation([Values]bool fromFile)
		{
			...
			{
				SilenceDeprecations = new List<string> { DeprecationId.GlobalBuiltin, DeprecationId.Import }
			};
			...
			Assert.AreEqual(
				...
				"   at prefix (Files/imports/warnings/custom-warning/sass/_mixins.sass:6:7) -> " +
